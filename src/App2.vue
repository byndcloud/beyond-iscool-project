    ...
    <v-main>
      ...
      <div class="messages-container">
        <div v-for="message in messages" :key="message">
          ...
          <!-- caso mais simples: -->
          <!-- 
            <v-card outlined class='ma-3' :class="{ 'teal accent-4': message.user == 'friend', 'grey lighten-2': message.user != 'friend' }">
              <v-card-subtitle class="font-weight-bold">{{ message.user == 'friend' ? name : "Eu" }}</v-card-subtitle>
              <v-card-text>{{ message.text}}</v-card-text>
            </v-card>
           -->
        </div>
